...
echo "启动 gRPC 服务端..."
python3 -u time_server.py --device cuda | tee tee_server.log &
SERVER_PID=$!
...
gramine-sgx ./pytorch time_client.py  --run-times "$RUN_TIMES" --input-len "$INPUT_LEN" | tee tee_client.log
