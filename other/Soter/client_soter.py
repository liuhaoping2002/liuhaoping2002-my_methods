...
class TransformerClient:
    def __init__(self):
        ...
        #et = time.time()
        #print(f"file cost {(et-st)*1000:.3f}")
        
        self.n_layer = int(data['n_layer'][0])
        
        self.c_proj_w = [data['c_proj_w'][i] for i in range(self.n_layer)]
        self.c_proj_b = [data['c_proj_b'][i] for i in range(self.n_layer)]
    ...
    def forward(self, i, state):
        ...
        while True:
            if local_i == 1:  # LN1
                ...
                obf_ir = self.scale*state['input']
                obf_ir = self.scale*state['ln1']
                ...
                obf_ir = self.scale*state['ff2']
                obf_ir = self.scale*state['output']
