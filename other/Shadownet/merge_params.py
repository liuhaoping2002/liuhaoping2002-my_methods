...
def merge_server_weights_to_client():
    ...
    CLIENT_PARAMS_DIR = 'gpt2_params_split'
    
    print(f"正在读取服务端总参数文件: {SERVER_PARAMS_PATH} ...")
    if not os.path.exists(SERVER_PARAMS_PATH):
        print("错误：未找到服务端参数文件。请确保 gpt2_server_params.npz 存在。")
        return
...
if __name__ == "__main__":
    # 为了安全，建议先备份 gpt2_params_split 文件夹
    if os.path.exists('gpt2_params_split'):
        if not os.path.exists('gpt2_params_split_backup'):
            print("正在创建参数备份 (gpt2_params_split_backup)...")
            shutil.copytree('gpt2_params_split', 'gpt2_params_split_backup')
