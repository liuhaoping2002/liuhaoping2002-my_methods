...
class TransformerClient:
    def __init__(self):
        ...
        data_s = np.load('gpt2_server_params.npz')
        
        self.c_attn_w = [data_s['c_attn_w'][i] for i in range(self.n_layer)]
        self.c_attn_b = [data_s['c_attn_b'][i] for i in range(self.n_layer)]

        self.c_proj_w = [data_s['c_proj_w'][i] for i in range(self.n_layer)]
        self.c_proj_b = [data_s['c_proj_b'][i] for i in range(self.n_layer)]

        self.mlp_c_fc_w = [data_s['mlp_c_fc_w'][i] for i in range(self.n_layer)]
        self.mlp_c_fc_b = [data_s['mlp_c_fc_b'][i] for i in range(self.n_layer)]

        self.mlp_c_proj_w = [data_s['mlp_c_proj_w'][i] for i in range(self.n_layer)]
        self.mlp_c_proj_b = [data_s['mlp_c_proj_b'][i] for i in range(self.n_layer)]

        self.lm_head_w = data_s['lm_head_w']
    ...
    def forward(self, i, state):
        ...
        if i == 1201:
            self.time_count = time_cost(f"i={i-1}",self.time_count)
            state['ln_final'] = layer_norm(state['input'], self.final_gamma, self.final_beta)
            self.time_count = time_cost(f"i={i}",self.time_count)
            return 1202, state
        ...
        while True:
            ...
            if local_i == 1:  # LN1
                ...
                scores = np.matmul(state['Q'], state['K'].transpose(0, 1, 3, 2)) / np.sqrt(self.head_dim)
                S_q = state['Q'].shape[2]
...
def perform_inference(client, stub, input_text, collect_times=True):
    ...
    if collect_times:
        ...
        print(f"Total time count: {time_totals * 1000:.3f} ms")
