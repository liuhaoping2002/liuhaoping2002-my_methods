...
SNAME="time_client_Translinkgurad.py"
# 脚本所在目录（保证能找到 python 文件）
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PYTHON_SCRIPT="$SCRIPT_DIR/$SNAME"
...
echo "启动 gRPC 服务端..."
#echo "lhp331."| sudo -S rmmod  nvidia_uvm
#echo "lhp331."| sudo -S modprobe nvidia_uvm

python3 -u time_server_Translinkguard.py --device cuda | tee tee_server.log &
SERVER_PID=$!
...
make clean
make SGX=1
#gramine-sgx ./pytorch time_client_goa.py  --run-times 3 --input-len 15 | tee tee_client.log
gramine-sgx ./pytorch time_client_Translinkgurad.py  --run-times "$RUN_TIMES" --input-len "$INPUT_LEN" | tee tee_client.log
