...
python3 -u time_server_goa.py --device cuda | tee tee_server.log &
SERVER_PID=$!
...
gramine-sgx ./pytorch time_client_goa.py  --run-times "$RUN_TIMES" --input-len "$INPUT_LEN" | tee tee_client.log
