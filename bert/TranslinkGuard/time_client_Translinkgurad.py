...
model_dim = 768
# --- 新增：安全缩放工具 ---
perm_gelu = np.random.permutation(model_dim)
def scale_state(state_np, local_i):

    new_state = {} 
    if local_i == 10:
        new_state['gelu'] = (state_np['gelu']+ np.random.randn(*state_np['gelu'].shape))[:,:,perm_gelu]
        new_state['attn_residual'] = state_np['attn_residual'] [:,:,perm_gelu]
        
    elif local_i == 12:
        new_state['ff2'] = (state_np['ff2'] + np.random.randn(*state_np['ff2'].shape))[:,:,perm_gelu]
...
class TransformerClient:
    ...
    def load_embeddings(self):
        # 注意：在实际运行中，需要确保 bert_params_split/embeddings.npz 存在
    ...
    for layer in range(client.n_layer):
        ...
        try:
            while True:
                ...
                # --- Server 传输逻辑 ---
                if i % 100 in [2, 9, 11] or i == 1202:
                    
                    keys_to_send = []
                    local_i = i % 100
                    
                    if local_i == 2: keys_to_send = ['ln1', 'input']
                    ...
                    req = demo_pb2.TransformerRequest(op_id=i, state=demo_pb2.State(items=np_to_state(req_state_np)))
                    ...
                    #print(f"return i : {i}")
                    if i%100 == 10 or i%100 == 12: 
                        t_scale_start = time.perf_counter()
                        decrypted_state = scale_state(received_state, i%100)
