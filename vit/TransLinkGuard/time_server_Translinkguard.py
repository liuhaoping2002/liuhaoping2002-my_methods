...
def layer_norm(x, weight, bias, use_cuda, eps=1e-5):
...
class TransformerService(demo_pb2_grpc.TransformerServiceServicer):
    def __init__(self, device_choice: str = "cpu"):
        ...
        try:
            ...
            raise FileNotFoundError("vit_server_params.npz not found. Run download_vit_params.py first.")
        ...
        client_data = np.load('vit_base_params/vit_params.npz') 
        ...
        ln2_gamma = [client_data['ln2_gamma'][i] for i in range(self.n_layer)]
        ln2_beta = [client_data['ln2_beta'][i] for i in range(self.n_layer)]
        ...
        if self.use_cuda:
            ...
            self.ln2_gamma = [torch.from_numpy(g).to(self.device) for g in ln2_gamma]
            self.ln2_beta = [torch.from_numpy(b).to(self.device) for b in ln2_beta]
        else:
            # 权重保持 numpy
        ...
        print(f"shape of c_proj_w: {self.c_proj_w[0].shape} ")
        print(f"shape of b_proj_w: {self.c_proj_b[0].shape} ")
    ...
    def Process(self, request, context):
        ...
        with context_manager:
            ...
            while True:
                ...
                if i >= 1202: # Logits
                    response_state['final_hidden'] = s['ln_final']  # [B, 197, 768]
                    i = 9999
                    break
                ...
                if local_i == 2:  # QKV
                    ...
                    if self.use_cuda:
                        ...
                        s['attn_out'] = np.dot(s['aout'], self.c_proj_w[current_layer]) + self.c_proj_b[current_layer][None, None, :]
                    t1 = time.perf_counter()
                    pure_compute_time += (t1 - t0)
                    i += 1
                ...
                #elif local_i == 7: # Add
                    t0 = time.perf_counter()
                    s['attn_residual'] = s['attn_out'] + s['input']
                    t1 = time.perf_counter()
                    pure_compute_time += (t1 - t0)
                    i += 1
                    #break
                    ...
                    t0 = time.perf_counter()
                    s['ln2'] = layer_norm(s['attn_residual'], self.ln2_gamma[current_layer], self.ln2_beta[current_layer], self.use_cuda)
