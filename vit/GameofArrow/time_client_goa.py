...
def obf_state_double(state_np, D):
    ...
    for i in state_np:
        s_reshape = state_np[i].reshape(1, state_np[i].shape[2], -1)
        new_state[i] = D[:, None]*s_reshape+D[:, None]*np.ones((s_reshape.shape[1],1))@np.random.rand(1, s_reshape.shape[2])
        #new_state[i] = state_np[i]
        #print("new_state[i].shape")
    return np_to_state(state_np)
...
class TransformerClient:
    def __init__(self):
        ...
        self.seq_len = 197
        self.scale = 1.834
        self.D_d = np.random.rand(self.hidden_size)
    ...
    def seq_params_init(self):
        self.D_seqlen = np.random.rand(self.seq_len)
    ...
    def forward(self, i, state, params):
        ...
        while True:
            ...
            elif local_i == 6: # Attn @ V
                attn_out = state['attn_out']
                i += 1 
                local_i += 1
            #    del state['K'],state['Q'],state['V'],state['scores'],state['attn']
                break
...
def perform_inference(client, stub, image_pil, profiler=None):
    ...
    client.seq_params_init()
    #print(f"Input image processed to embeddings with shape: {hidden.shape}")
    # [统计] Embedding 时间
    #if profiler:
        #profiler.embedding_time = 0.15  # 模拟，或用 time.perf_counter() 包起来
