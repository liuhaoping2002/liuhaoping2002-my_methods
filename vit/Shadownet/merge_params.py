...
def merge_server_weights_to_client():
    # 路径配置
    SERVER_PARAMS_PATH = 'vit_server_params.npz'
    CLIENT_PARAMS_DIR = 'vit_params'
    
    print(f"正在读取服务端总参数文件: {SERVER_PARAMS_PATH} ...")
    if not os.path.exists(SERVER_PARAMS_PATH):
        print("错误：未找到服务端参数文件。请确保 vit_server_params.npz 存在。")
        return
...
if __name__ == "__main__":
    # 为了安全，建议先备份 vit_params_split 文件夹
    if os.path.exists('vit_params'):
        if not os.path.exists('vit_params_split_backup'):
            print("正在创建参数备份 (vit_params_split_backup)...")
            shutil.copytree('vit_params', 'vit_params_split_backup')
